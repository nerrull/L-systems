...
class GrowthModule extends Module{
    ...
    String next;
    GrowthModule(String name, float [] params,String n  ){
      super(name, params);
      this.next= n;
...
class GrowingPlant extends ParametricLSystem{
  //Root parameters
  ...
  int maxOrder, nBranches;
   ...
   GrowingPlant(){
    ...
    this.lb = new float[]{10,5,2};
   //Length of interbranch zone
    this.ln = new float[]{20,5,5};
        //Length of apical zone

    this.la = new float[]{10,5,5};
    this.axial_SD = new float[]{radians(2),radians(2),radians(2)};
    this.offShootAngles = new float[]{radians(18),radians(18),radians(18)};

    defineRules();
    word = new Word();

    GrowthModule m = new S(0,0, lb[0],0, "branching_zone");
  ...
  float growth(float time, float maxLength){
    float len = maxLength*(1-exp(-growthSpeed/(maxLength-1)*time));
    return len;
  }
  ...
  */
  ...
  ParametricRule DelayRule = new ParametricRule() {
    public ArrayList<Module> rule(Module m) { 
      ArrayList<Module> ret = new  ArrayList<Module>();
      GrowthModule rm = (GrowthModule) m;
      float t = m.getP(0);
  ...
  boolean a_condition(float l, float maxLength, float t, float delta_x, float delta_t){
    return (l + delta_x<= growth(t+delta_t, maxLength));
  }
  ...
  ParametricRule SegmentRule = new ParametricRule() {
    public ArrayList<Module> rule(Module m) { 
      ...
      float c1 = l+delta_x;
      float c2 = growth(t+delta_t, maxLen);
      ...
      //A & (l+delta_x ≥ ls): R F(ls−l) Nst+delta_t−(tend−t0)
  ...
  ParametricRule BranchingRule = new ParametricRule() {
    public ArrayList<Module> rule(Module m) { 
      ...
      float delay =ln[order]/delta_t *random(1, 100);
